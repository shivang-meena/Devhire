...
function filterapplications(type) {
    ...
    if (type === "All") {
        ...
        const filtered = applications.filter((app) => app.status === type);
        setapplicationsdata(filtered);
    }
}
// useEffect(()=>{filterapplications("All")},[]);
            ...
            <div>
                ...
                <div className="flex flex-col h-[80vh] overflow-y-auto">
                      {Applicationsdata&&Applicationsdata.map((application)=>{
                        return <Applicationcard jobid={application.jobid} key={application.id} role={application.role} company={application.company} location={application.location} applydate={application.postedDate} status={application.status}/>
                      })}
                </div>
            </div>
        </div>
